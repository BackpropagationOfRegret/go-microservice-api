...
PENDING → PAID → DELIVERING → DELIVERED
    ↓
CANCELLED  (только при payment.failed)
```
...
В сагах «откат» — это не `ROLLBACK` across databases, а **компенсирующие действия**.

### Что реализовано

| Ситуация | Компенсация |
|----------|-------------|
| Оплата не прошла (`payment.failed`) | `order-svc` → статус `CANCELLED` |

Пользователь видит отменённый заказ. Деньги не списывались — возврат не нужен.

### Что не реализовано (но нужно в проде)

| Ситуация | Что должно быть |
|----------|-----------------|
| Оплата прошла, кухня не может выполнить заказ | Возврат (`refund`) в payment-svc + отмена заказа |
| Курьер не найден | Отложить / отмена / возврат |
| Дубликат события из Kafka | Идемпотентность (обработать `order_id` только один раз) |
| Сбой после списания, до `payment.processed` | Повторная обработка или ручная сверка |

Полноценный rollback в хореографии обычно — **новые компенсирующие события** (`payment.refunded`, `order.cancelled`), а не откат SQL в чужой базе.
...
```bash
make up       # Docker: все сервисы + Kafka + Postgres + frontend
make docs     # ссылки на API docs и UI
make demo     # быстрый тест API
```
...
1. **Хореография без оркестратора** — сложно видеть целую сагу и делать сложные откаты.
2. **Нет outbox / inbox** — риск рассинхрона БД и Kafka.
3. **Нет идемпотентности** — повтор события может привести к дублям (для payment частично защищает `UNIQUE order_id`).
4. **Один компенсирующий сценарий** — только отмена при failed payment.
5. **HTTP вместо gRPC** между сервисами — ок для демо.
6. **notification-svc** только логирует, реально не шлёт SMS/push.
7. **Симуляция** времени готовки и доставки (`sleep` в коде).
